...
class Logger(logging.Logger):
    def __init__(self, name: str, path:str, console_level:int=logging.WARNING, file_level:int=logging.DEBUG, file_suffix:str='') -> None:
        ...
        sh_formatter = ColorFormatter('%(asctime)s.%(msecs)02d [%(name)s] %(levelname)s %(message)s', '%H:%M:%S')
        fh_formatter = logging.Formatter('%(asctime)s.%(msecs)02d [%(name)s] %(levelname)s %(message)s', '%m-%d %H:%M:%S')
